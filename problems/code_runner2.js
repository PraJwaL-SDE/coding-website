    ...
    success: function(data) {
        ...
        $.ajax({
            ...
            success : (data)=>{
                ...
                console.log(data.empty_code)
                  editor = CodeMirror.fromTextArea(textBoxCode, {
                    lineNumbers: true,
                    value: data.empty_code,
                    mode: "text/x-java",
                    theme: "material",
                    extraKeys: {
                  ...
                  var fontSize = 18; // Adjust font size as needed
                    editor.getWrapperElement().style.fontSize = fontSize + "px";
                
                    // Code hints
  CodeMirror.registerHelper("hint", "text/x-java", function(editor) {
    var word = /[\w$]+/;
    ...
    var list = [
      ...
      "Math", "ArrayList", "HashMap", "HashSet", "LinkedList", "Queue", "Stack", "Scanner", 
      "PrintStream", "File", "FileReader", "FileWriter"
    ]; // Add more keywords here
    var hints = [];
    for (var i = 0; i < list.length; i++) {
      if (list[i].indexOf(curWord) === 0) hints.push(list[i]);
    }
    return {list: hints, from: CodeMirror.Pos(cur.line, start), to: CodeMirror.Pos(cur.line, end)};
  });

  // Show hints
  editor.on("keyup", function(cm, event) {
    if (!cm.state.completionActive && event.keyCode != 13 && event.keyCode != 27 && event.keyCode != 32 && event.keyCode != 17 && !event.ctrlKey && !event.metaKey && !event.shiftKey && !event.altKey) {
